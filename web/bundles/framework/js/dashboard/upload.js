$(function(){
	...
	function build_unit_type_dropdown(el) {
	...
	$(".dropdown-menu").on('click', 'li a', function(e){
      ...
      $(this).parent().parent().parent().children(".btn").val($(this).text());
    ...
    function _clean_cat_list(cat_level) {
    	var i;
    ...
    function build_cat_list (cat_level, cat_id, json_text) {
		...
		if(sub_cats.length > 0) {
			...
			div_searchlist_wrapper.on('click', 'a', function(){
				...
				build_cat_list(cat_level+1, sub_cats[$(this).attr("tabindex")].id, sub_cats[$(this).attr("tabindex")].c);
			});
		} else {
			build_product_detail(cat_id);
		}
    } 
    
    build_cat_list(0, 1, cat_structure);
    
    
    
    
    function _build_textbox(cat_id, col_id) {
    	var temp = 
    ...
    function _build_select_box(cat_id, col_id) {
    ...
    function _build_select_box_place_of_origin(cat_id, col_id, place_of_origin) {
    	var temp = 
			'<div class="form-group">'+
			  '<label class="col-md-4 control-label" for="textinput">'+_AsgUtil.Mapping.getColumnName(cat_id, col_id)+'</label>'  +
			  '<div class="col-md-6">';
    ...
    function build_product_detail(cat_id) {
    	...
    	$.each(current_product_detail_list, function(i) {
    		if(current_product_detail_list[i].n === "Place of Origin") {
    			_build_select_box_place_of_origin(cat_id, i, country_mapping);
    		} else if (current_product_detail_list[i].def_v.length > 0){
    			_build_select_box(cat_id, i);
    		} else {
    			_build_textbox(cat_id, i);
    		}
    	}); 
    }
